...
class Widget_Archive extends Widget_Abstract_Contents
{
    ...
    public function getTotal()
    {
        if (false === $this->_total) {
        	 $sql = Cache_Main::get('countsql');
        	 if(isset($sql[Typecho_Router::$snapshot])) $this->_total = $sql[Typecho_Router::$snapshot];
        	 else
        	 {
              $this->_total = $this->size($this->_countSql);
              $sql[Typecho_Router::$snapshot] = $this->_total;
              Cache_Main::set('countsql', $sql);
            }
        }
    ...
    public function theNext($format = '%s', $default = NULL, $custom = array())
    {
        /*$content = $this->db->fetchRow($this->select()->where('table.contents.created > ? AND table.contents.created < ?',
            ...
            ->limit(1));*/
        $content = NULL;
        foreach(Cache_Main::$_sections['postlink'] as $key => $post)
        {
        	if($post[0] == $this->cid)
        	{
                $count = count(Cache_Main::$_sections['postlink']);
        		while((++$key) < $count) {
        			$data = Cache_Main::$_sections['postlink'][$key];
        			if($data[4] == 'publish' && $data[5] == $this->type)
        			{
        				$content = array('cid' => $data[0], 'title' => $data[1], 'permalink' => $data[2], 'pathinfo' => $data[3]);
        				break;
        			}
        		}
        		break;
