...
class Widget_Options extends Typecho_Widget
{
    ...
    {
        $cached = Cache_Main::get('options');
        
        if(empty($cached))
        {
            $this->row = array();
            $keyvalues = $this->db->fetchAll($this->db->select()->from('table.options')->where('user = 0'));
            foreach($keyvalues as $eachkeyvalue) $this->row[$eachkeyvalue['name']] = $eachkeyvalue['value'];

            Cache_Main::set('options', $this->row);
        }
        else
        {
            $this->row = $cached;
		...
		/** 自动初始化路由表 */
		if(!is_array($this->row["routingTable"]))
		{
			$this->row["routingTable"] = unserialize($this->row["routingTable"]);
			if (!isset($this->row["routingTable"][0])) {
				/** 解析路由并缓存 */
				$parser = new Typecho_Router_Parser($this->row["routingTable"]);
				$parsedRoutingTable = $parser->parse();
				$this->row["routingTable"] = array_merge(array($parsedRoutingTable), $this->row["routingTable"]);

				$this->db->query($this->db->update('table.options')->rows(array('value' => serialize($this->row["routingTable"])))
