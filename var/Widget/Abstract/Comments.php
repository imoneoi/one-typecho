...
class Widget_Abstract_Comments extends Widget_Abstract
{
    ...
    {
    	/*if(!isset(self::$_parentContentCache[$this->cid]))
    	{
        	self::$_parentContentCache[$this->cid] = $this->db->fetchRow($this->widget('Widget_Abstract_Contents')->select()
        		->where('table.contents.cid = ?', $this->cid)
        		->limit(1), array($this->widget('Widget_Abstract_Contents'), 'filter'));
       	}
       	return self::$_parentContentCache[$this->cid];*/
       	foreach(Cache_Main::$_sections['postlink'] as $post)
       	{
       		if($post[0] == $this->cid) return array('title' => $post[1], 'permalink' => $post[2], 'pathinfo' => $post[3]);
       	}
    }
