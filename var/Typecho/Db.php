...
class Typecho_Db
{
    ...
    public function query($query, $op = self::READ, $action = self::SELECT)
    {
        ...
        /** Cache Hook **/
        if($action == self::INSERT) $lastInsertId = $this->_adapter->lastInsertId($resource, $handle);
        else $lastInsertId = NULL;
        
        if($writeDb) Cache_Plugin::on_writedb($query, $lastInsertId);
        
        /** Performance Counter **/
        Cache_Plugin::$_db_queries++;

        /*if(defined("__TYPECHO_PROFILE__")) echo '<!--[Query #', Cache_Plugin::$_db_queries, '] ['.$query."]-->\n\r\n\r";*/
