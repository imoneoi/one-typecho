        ...
        foreach (self::$_routingTable as $key => $route) {
            if (preg_match($route['regx'], $pathInfo, $matches)) {
                ...
                try {
					...
					{
						$snap .= ';';
						foreach($params as $param_name => $param_value) $snap .= $param_name.'='.$param_value.',';
						$snap = substr($snap,0,-1);
					}
					self::$snapshot = $snap;
					...
					if(@in_array(self::$current, $advoptions['static']))
					{
						$snap = str_replace(array('_','"',"'",'*','?','<','>',':',"\\",'/','|'), array('__','_0','_1','_2','_3','_4','_5','_6','_7','_8','_9'), $snap);
						$isCached = true;
						$dir = Cache_Main::$_cacheDir . 'static/';
						if(!is_dir($dir)) mkdir($dir);
						$dir = $dir.$snap.'.htm';
						if(file_exists($dir) && (!$advoptions['sexpire'] || (time() - filemtime($dir)) <= $advoptions['sexpire']))
                    	{
					...
					if($isCached) @file_put_contents($dir, ob_get_contents());
