...
$options->generator = 'OneTypecho ' . Typecho_Common::VERSION;
        ...
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <?php
                                    ...
                                    try {
                                        ...
                                        $installDb->query($installDb->insert('table.options')->rows(array('name' => 'advanced', 'user' => 0, 'value' => Json::encode( array('static' => array(), 'sexpire' => 86400) ))));
                            ...
                            if ('config' == _r('action')) {
                                ...
                                if (!isset($config) && $success && !_r('created')) {
                                    ...
                                    try {
                                        ...
                                        . _t('对不起，无法连接数据库，请先检查数据库配置再继续进行安装') . '<br>' . $e->getMessage() . '</p>';
                                    } catch (Typecho_Db_Exception $e) {
                                        $success = false;
                                        echo '<p class="message error">'
...
<p><button name="created" value="1" type="submit" class="btn primary">创建完毕, 继续安装 &raquo;</button></p></div>
            ...
            <?php  else: ?>
                ...
                <?php if(!function_exists("apc_cache_info") && !function_exists("apcu_cache_info")) { 
                    /** Test file R&W **/
                    $Fn = __TYPECHO_ROOT_DIR__ . '/usr/filerw.test';
                    @file_put_contents($Fn, "write_test");
                    $ok = @file_get_contents($Fn) == "write_test";
                    @unlink($Fn);
                    /********************/
                    if(!$ok) {
                        ?><p class="message error"><?php _e('文件 / APC缓存不可用，缓存机制将完全失效。'); ?></p><?php
                    }
                    else {
                        ?><p class="message notice"><?php _e('APC缓存不可用，将使用文件缓存。'); ?></p><?php
                    }
                 ?><?php } ?>
                <h2><?php _e('许可及协议'); ?></h2>
